...
class product
{
    ...
    public function getFeaturedProducts()
    {
     ...
     // get product sold max
    public function getProductSoldMax()
    {
        $query = "SELECT * FROM smartphonestore.products where products.soldCount = 
        (select max(products.soldCount) FROM products)";
        $ex_query = $this->db->select($query);
        if ($ex_query) {
            $result = mysqli_fetch_all($this->db->select($query), MYSQLI_ASSOC);
            return $result;
        }
    ...
    public function getProductsByCateId($page = 1, $cateId, $total = 4)
    {
        if ($page <= 0) {
            $page = 1;
        }
        ...
        $query =
            "SELECT *
			 FROM products
			 WHERE products.status = 1 AND products.cateId = $cateId
             LIMIT $tmp,$total";
        $mysqli_result = $this->db->select($query);
        if ($mysqli_result) {
